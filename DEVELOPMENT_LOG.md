...
- **改動重點**：
  ...
  - **新增主題【常見問答 (Q&A)】**：整併原「網格轉 CAD (Mesh to STEP) 與實體縫合問題排查指南」，並新增近期工程驗證之 7 大核心疑難問題與標準作業程序 (SOP)。
  - **首頁互動目錄 (Table of Contents)**：在 Q&A 分頁首頁設計可點擊之目錄索引，支援平滑跳轉（Smooth Scroll）直達問題卡片，各卡片配備「🔝 返回目錄」快速按鈕。
  - **完整收錄近期實例**：包含 CFD 匯入 IGES 提示為三角網格 (.glb) 非 CAD 實體之根因剖析與 3 種解法、STEP 實體縫合、風洞流體域、index.html 下載異常、CAD HUD 預覽卡片、HF 節點逾時與分享連結等 SOP。
