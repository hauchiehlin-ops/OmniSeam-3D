...
- **改動重點**：
  - **全域多語系在地化徹底稽核與硬編碼文字根除**：
    - 修復目標格式卡片（`TargetFormatCard.tsx`）在非中文語系下仍顯示繁中之問題，將快速標籤（`3D 列印`、`工業 CAD`、`彩色/結構`、`網頁/AR`、`通用網格`）與切換提示全數改為 `t('settings.quick_badge_*')` 與 `t('settings.all_formats_interchangeable')` 動態鍵值。
    - 空間姿態旋轉工具列（`ModelTransformToolbar.tsx`）：三軸 90° 逆/順時針翻轉、180° 翻轉及關閉面板 Tooltip 全數在地化。
    - 行動端 AR 預覽視圖（`ArPreviewModal.tsx` & `MobileArView.tsx`）：Localhost 連線提示、預設目標重置、無效連線錯誤、3D 手勢指引與模型加載狀態全數在地化。
    - 風洞流體域抽取視窗（`WindTunnelModal.tsx`）：滑桿尺度指示（緊密、標準、充裕、標準尾流、標準無干擾）全數在地化。
    - 幾何體檢報告（`AuditReport.tsx`）：零厚度開放曲面雙途徑解決方案、拓撲健康/缺陷回退文字、切片機非水密警告訊息全數在地化。
    - 前端轉換管線（`client-pipeline.ts`）與動態標題（`App.tsx`）：移除寫死之二元 `isZh` 分支，透過 `i18n.t` 支援全 6 國語系即時進度狀態文字；網頁標題動態翻譯。
    - 六國語系字典檔完整注入：`zh-TW`、`zh-CN`、`en`、`ja`、`ko`、`th` 新增鍵值全數補齊，經 TypeScript 編譯與 Vite 打包驗證零錯誤。
  - **操作手冊全面同步更新**：
    - **新增 Q&A Case 11**：3D 空間姿態旋轉與頂點矩陣永久烘焙（切片機與 CAD 姿態對齊、Y-Up vs Z-Up 軸系差解決方案）。
    - **新增 Q&A Case 12**：行動端 AR 實物 1:1 投影、2小時 TTL 臨時中繼與 Localhost IP 跨網連線排查 SOP。
    - **擴充 3D 視圖與工具章節**：新增「3D 空間姿態旋轉與頂點永久烘焙」、「多軸空間座標系（Cartesian、Polar、Cylindrical、Spherical）與方位羅盤 Gizmo」、「跨裝置 AR 實物 1:1 投影」三大核心功能圖文指南。
    - **快速開始章節同步**：Step 2 更新獨立目標格式卡與預設修復策略導引；Step 3 更新純檢視工具、雙屏比較，並說明內建基準測試模型已全數移除以保護使用者隱私並避免覆蓋。
  - **隱私權與資料安全政策同步更新**：
    - **新增第 5 大安全支柱**：AR 投影中繼會話 2 小時 TTL 自動銷毀機制（Zero-Log 原則）。
    - **新增保證條款**：100% 前端幾何變換與頂點矩陣烘焙，純客戶端記憶體運算，絕無幾何資料外傳。
    - **生命週期對照表更新**：新增「AR 行動中繼會話」於 Pure Client 與專屬 Space 節點的處理與自動銷毀方式。
  - **六國語系全數同步**：繁中 (`zh-TW`)、簡中 (`zh-CN`)、英文 (`en`)、日文 (`ja`)、韓文 (`ko`)、泰文 (`th`) 字典鍵值無遺漏更新。

### `v3.5.0` (2026-09-06) - 擴充 Q&A 10 大排查 SOP (檔案用途定位與 CFD 15 秒幾何探測機制) & 多階水密修復增強
- **改動重點**：
  - **Q&A 擴充至 10 大排查 SOP 互動目錄**：
    - **檔案用途定位 (Case 3 深度升級)**：詳解 `_omniseam.step` (待測模型本體實心零件，CAD/FEA/壁面條件用，不可作流體域) vs. `_fluid_domain.step` (外部風洞流道空間，CFD 數值模擬專用) vs. `_polyheal.glb` (視覺預覽/包裹) 的幾何本質差異與 CFD 標準載入流程。
    - **幾何品質的前置影響 (Case 8)**：說明非流體幾何、反向法向量、非流形邊界對風洞布林運算的破壞與預處理要點。
    - **風洞抽取聯動右側設定 (Case 9)**：解析風洞模態與右側雲端運算、修復策略及簡化參數的深度依賴關係。
    - **CFD 15 秒幾何探測機制 (Case 10)**：深度剖析 CFD 軟體內部以獨立進程調用 OpenCASCADE (`-parse_and_exit`) 進行幾何預探測以防止當機的安全熔斷機制，解構為何由網格直轉的 Faceted STEP 易觸發 15 秒超時，並提供 2 大標準避坑操作流程 (SOP)。
  - **多階水密自動修復與 STEP 實體匯出嚴格驗證**：修復網格孔洞定向修補演算法，嚴格檢驗 `MANIFOLD_SOLID_BREP` 與拓撲閉合。
  - **六國語系同步**：繁中、簡中、英文、日文、韓文、泰文全語系字典檔同步更新完成。

### `v3.4.0` (2026-09-06) - 手冊分頁改下拉選單、新增 Q&A 主題與 7 大排查 SOP 互動目錄
- **改動重點**：
  - **操作說明手冊分頁重構**：為避免標籤過多擠壓版面，將手冊主題導航由水平多欄按鈕升級為「下拉式選單（Dropdown Select）」呈現，支援章節圖示聯動與上一章/下一章翻頁導航。
  - **新增主題【常見問答 (Q&A)】**：整併原「網格轉 CAD (Mesh to STEP) 與實體縫合問題排查指南」，並建構 7 大核心疑難問題與標準作業程序 (SOP)。
  - **首頁互動目錄 (Table of Contents)**：在 Q&A 分頁首頁設計下拉式目錄索引，支援平滑跳轉（Smooth Scroll）直達問題卡片，各卡片配備「🔝 返回目錄」快速按鈕。
...
### `v4.1.0` (2026-09-07) - 3D 空間姿態旋轉烘焙、預覽版面無遮擋重構與多維空間座標系支援
- **主要架構與新功能**：
  - **3D 物件空間旋轉姿態調整與永久頂點烘焙**：
    - 雙方案並存：方案 A（X/Y/Z 精確度數輸入、-15°/+15° 微調、-90°/+90°/180° 一鍵翻轉、一鍵重置）與方案 B（Three.js TransformControls 3D 互動控制環 Gizmo），兩者即時雙向連動。
    - 幾何中心矩陣烘焙：純前端離線引擎（`GeometricKernel.rotateMesh`）與雲端後端（`router.py` Trimesh 旋轉變換）在匯出時將旋轉角度直接烘焙至幾何頂點與法向量，確保匯出的 STL、STEP、OBJ、3MF、GLB 在第三方軟體中永久保留旋轉樣態。
  - **預覽區版面重構（徹底消除對 3D 物件主體之遮擋）**：
    - 將旋轉面板改為外置式橫向控制抽屜（Horizontal Dock Bar），置於頂部工具列下方，100% 釋放 3D 畫布內部空間，立體與細長模型完全無遮擋。
  - **多維空間座標系系統（4 大座標系 + 智慧推薦 + 手動自由切換）**：
    - 新增 `CoordinateSystemManager`，支援笛卡爾直角座標系（Cartesian XYZ）、極座標系（Polar）、圓柱座標系（Cylindrical）、球座標系（Spherical）與純淨隱藏模式。
    - 智慧幾何特徵自動識別（Auto-Adaptive）：依據模型外接包圍盒尺寸比例與旋轉軸對稱性自動推薦最適座標系，並支援在工具列自由手動切換覆寫。
    - 視角角落 3D 方向羅盤儀（Orientation Compass Gizmo）：右下角常駐 XYZ 姿態羅盤，隨相機視角即時同步旋轉。

### `v4.0.0` (2026-09-07) - 核心互動體驗重構（格式卡片獨立高亮 + AI 引擎自適應聯動 + 樣品功能清理）
- **主要架構與介面重構**：
  - **獨立高亮「目標匯出格式」卡片**：新增 `TargetFormatCard.tsx`，將目標格式從參數設定內部抽離並獨立置頂於「轉換與修復參數設定」卡片之上，輔以微漸變環境光暈、HIGHLIGHT 徽章與常見格式（STL、STEP、3MF、GLB、OBJ）快捷切換膠囊。
  - **動態響應「AI 智慧核心推薦」**：解決切換 Pure Client 與 Cloud Server 時描述未更新的問題；重構為即時動態連動，切換引擎或載入不同副檔名模型時即刻更新該模式的算力優勢、幾何縫合支援與資安說明。
  - **精簡頂部導航列**：移除頂部重複的「幾何健康體檢報告」置頂按鈕，統一由右側參數面板底部的「僅進行幾何體檢診斷 (Inspect Only)」啟動體檢彈窗。
  - **清理基準測試樣品模型功能與無用代碼**：徹底移除「或載入基準測試模型」區塊，避免使用者載入個人模型後因誤點而覆蓋檔案；移除前端 `client.ts` 內近百行 STL 硬編碼樣品生成代碼及後端 `/sample/{sample_type}` 路由。
